...
def main():
    ...
    alleles = input_file['allele'].unique()
    if ";" in alleles[0]:
        alleles = alleles[0].split(";")

    with open('mhcnuggets_peptides.csv', 'w+') as f:
        for _, row in input_file.iterrows():
            f.write(row['peptide']+'\n')
        #get first entry in allele column
    ...
    for allele in alleles:
        ...
        predict(class_=args.mhcclass, peptides_path = 'mhcnuggets_peptides.csv', output=tmp_file_name, mhc=allele, rank_output=True)
    ...
    # Clean up
    os.remove('mhcnuggets_peptides.csv')
