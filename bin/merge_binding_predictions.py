...
# Create a logger
logging.basicConfig(
    filename="merge_prediction_outputs.log",
    filemode="w",
    level=logging.DEBUG,
...
def parse_args(argv=None) -> typing.List[str]:
    ...
    parser.add_argument("--syfpeithi_threshold", type=int, help="Threshold for syfpeithis relative max matrix score")
...
class PredictionResult:
    ...
    def _format_mhcflurry_prediction(self, threshold=None):
        ...
        df.columns = [
            f"mhcflurry_percentile_{mhcgnomes.parse(col).to_string()}" if col != "sequence" else col
    ...
    def _format_mhcnuggets_prediction(self, threshold=None):
        ...
        # Represent official allele names
        df.columns = [
            f"mhcnuggets_rank_{mhcgnomes.parse(col).to_string()}" if col != "sequence" else col for col in df.columns
        ]
        if threshold:
            # keep peptide if at least one allele has a score <= threshold
            df = df[~df.iloc[:, 1:].gt(threshold).any(1)]
...
def main():
    ...
    metadata = metadata.rename(columns={"peptide": "sequence"})
